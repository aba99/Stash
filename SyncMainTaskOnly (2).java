...
public class SyncMainTaskOnly {
	...
	{		
	    ...
	    HashMap<String,Session> ref_sess_map =  new HashMap<String,Session>();
		...
		for(String connKey:DUAPIConnections.keySet())
		{
			 ...
			 DuApiConnection conn=DUAPIConnections.get(connKey);
				 ...
				 if(conn.getConnName().substring(0,conn.getConnName().indexOf("/")).toUpperCase().equalsIgnoreCase(refnode))
				 {
					...
					ref_sess_map = conn.getSessionsHashMap_from_outside();
					
					Set<String> ref_Table = new HashSet<String>(ref_tasks_map.keySet());
					
					
					for(String tsk:ref_Table)
					{
						String session = ref_tasks_map.get(tsk).getSessionName();
						
						if(!sessionFilters.containsKey(session)|| !ref_tasks_map.get(tsk).getUprocName().equalsIgnoreCase(ref_sess_map.get(session).getHeader()))
						{
