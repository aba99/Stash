...
import com.orsyp.api.session.SessionModel;
    	  ...
    	  if(mainTasks.size()==1) 
       	  {
    		  ...
    		  if(mainTasks.get(0).getTaskType().equals(TaskType.Provoked))
    		  {
				...
				obj.create();
				System.out.println("[OPT TSK]  [" + oTskName+ "] ON NODE=[" + this.getConnName() + "] ---> OK");
...
public  void normalizeSchedules(String uprocname,ArrayList<String> listOfNewRules) throws UniverseException
{
	...
	ArrayList<String>currentRules = new ArrayList<String>(getRuleListFromTask(table.get(t)));

	listOfNewRules.removeAll(currentRules);
    ...
    for(int rule=0;rule<listOfNewRules.size();rule++)
    {
    	if(!this.ruleAlreadyExists(listOfNewRules.get(rule)))
    	{
    		...
    		createRule(listOfNewRules.get(rule));
    	}
    }
